<section class="bg-white rounded-sm shadow p-4 mx-4 mb-4">
    ...
    <table class="table w-full" aria-describedby="my-table">
        ...
        <tr>
            ...
            <td class="text-right text-green-600 p-1">{{ number_format($this->amount->total_income) }}</td>
        </tr>

        <tr>
            <td class="p-1">Pengeluaran</td>
            <td class="text-right text-red-600 p-1">{{ number_format($this->amount->total_spending) }}</td>
        </tr>

        <tr class="bg-yellow-100">
            <td class="p-1">Selisih</td>
            <td class="text-right font-bold p-1">
                {{ number_format($this->amount->total_income - $this->amount->total_spending) }}
            </td>
        </tr>
    </table>
