<div>
    @foreach ($transactionRecent as $key)
        @if ($key->date !== mktime(0, 0, 0, date('m'), date('j'), date('Y')))
            <section
                ...
                <div class="flex justify-center">
                    <table class="basis-11/12">
                        <tr class="border-b">
                            ...
                            <td class="w-6/12 text-right px-2 text-green-500">{{ number_format($key->total_income) }}
                            </td>
                        </tr>
                        <tr class="border-b">
                            <td class="w-6/12 px-2">Pengeluaran</td>
                            <td class="w-6/12 text-right px-2 text-red-500">{{ number_format($key->total_spending) }}
                            </td>
                        </tr>
